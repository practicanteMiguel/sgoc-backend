...
import {
  Requisicion,
  EstadoRequisicion,
} from '../../consumables/entities/requisicion.entity';
...
export class DotacionesService {
  constructor(
    @InjectRepository(DotacionSpace)
    private spaceRepo: Repository<DotacionSpace>,
    @InjectRepository(SolicitudDotacion)
    private solicitudRepo: Repository<SolicitudDotacion>,
    @InjectRepository(ReposicionDotacion)
    private reposicionRepo: Repository<ReposicionDotacion>,
    @InjectRepository(DotacionImagen)
    private imagenRepo: Repository<DotacionImagen>,
    ...
    @InjectRepository(Indumentaria)
    private indumentariaRepo: Repository<Indumentaria>,
  ...
  async createSolicitud(
    ...
    const reposicionesResult: Array<
      ReposicionDotacion & { imagenes: DotacionImagen[] }
    > = [];

    for (let i = 0; i < dto.reposiciones.length; i++) {
      const r = dto.reposiciones[i];

      const empleado = await this.employeeRepo.findOne({
        where: { id: r.empleado_id },
      });
      if (!empleado)
        throw new BadRequestException(
          `Empleado ${r.empleado_id} no encontrado`,
        );
      ...
      if (archivos.length) {
        ...
        for (const file of archivos) {
          const { url, public_id } = await this.cloudinary.uploadFull(
            file,
            folder,
          );
          imagenes.push(
  ...
  async getSolicitudes(token: string) {
    ...
    return this.solicitudRepo.find({
      ...
      relations: [
        'campo',
        'reposiciones',
        'reposiciones.empleado',
        'reposiciones.imagenes',
      ],
  ...
  async generarRq(solicitudId: string, dto: CreateRqDesdeDotacionDto) {
    ...
    if (solicitud.estado !== EstadoSolicitudDotacion.AUTORIZADA) {
      throw new BadRequestException(
        'La solicitud debe estar autorizada para generar una RQ',
      );
    }

    const conflicto = await this.rqRepo.findOne({
      where: { numero_rq: dto.numero_rq },
    });
    if (conflicto) {
    ...
    for (const item of dto.items) {
      const indumentaria = await this.indumentariaRepo.findOne({
        where: { id: item.indumentaria_id },
      });
      if (!indumentaria)
        throw new NotFoundException(
          `Item de indumentaria ${item.indumentaria_id} no encontrado`,
        );
  ...
  // RQ de dotacion creada directamente por el encargado, sin una solicitud/reposicion
  // detras (ej. dotacion inicial o periodica de todos los trabajadores).
  ...
  async firmarAutorizador(
    id: string,
    file: Express.Multer.File,
    dto: FirmaAutorizadorDto,
  ) {
    const solicitud = await this.solicitudRepo.findOne({
      where: { id },
      relations: ['campo'],
